...
class UserService:
    ...
    def update(self, user_id: str, account_id: str, item: PutUser) -> dict[str, Any] | None:
        cog_user = self.cog_wrapper.get_user(item.email)
        if not cog_user:
        ...
        self.repo.update(user_id, account_id, updates)
        new_item = self.repo.get(user_id, account_id)
        if not new_item:
    ...
    def update_user_avatar_url(self, user_id: str, account_id: str, item: PutUserAvatar) -> dict[str, Any] | None:
        ...
        if not existing:
            return None
        self.repo.update(user_id, account_id, {"avatar_url": item.avatar_url})
        new_item = self.repo.get(user_id, account_id)
        if not new_item:
            raise ValueError(f"User {user_id} not found after update.")
        return self._map_user(new_item, get_presigned_url=False)

    def get_user_metrics(self, user_id: str, account_id: str) -> dict[str, Any] | None:
        item = self.repo.get(user_id, account_id)
        if item:
            return item.get("user_metrics", {})
        return None

    def update_metrics(self, user_id: str, account_id: str, put_user_metrics: PutUserMetrics) -> dict[str, Any] | None:
        existing = self.repo.get(user_id, account_id)
    ...
    def generate_presigned_urls(self, user_id: str, account_id: str, files: list[FileSpec]) -> dict[str, dict[str, str]]:
        presigned_urls = {}
    ...
    def get_late_arrives(self, user_id: str, account_id: str) -> list[dict[str, Any]]:
        items = self.repo.list_all(account_id)
    ...
    def send_christmas_greetings(self, account_id: str) -> None:
        users = self.list_users(account_id, include_disabled=False)
        for user in users:
