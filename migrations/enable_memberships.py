#!/usr/bin/env python3
"""
Script: Enable Memberships

This script allows enabling memberships for a specific user or an entire account.
It is the reverse operation of disable_memberships.py.

Usage:
    python migrations/enable_memberships.py --user-id USER_ID                                     # Enable all for user
    python migrations/enable_memberships.py --account-id ACCOUNT_ID                               # Enable all for account
    python migrations/enable_memberships.py --account-id ACCOUNT_ID --except-user-ids ID1,ID2     # Exclude users
...
class MembershipEnabler:
    ...
    def enable_membership(self, pk: str, sk: str):
        """Set membership status to active"""
        if self.dry_run:
            print(f"   [DRY RUN] Would enable membership {pk} / {sk}")
            return True
        ...
        try:
            ...
            print(f"   ✅ Enabled membership {pk} / {sk}")
            return True
        except Exception as e:
            print(f"   ❌ Error enabling membership {pk} / {sk}: {e}")
            self.stats["errors"] += 1
            return False

    def run(self, user_id: str = None, account_id: str = None, except_user_ids: List[str] = None):
        """Run the enable process"""
        mode = "DRY RUN" if self.dry_run else "EXECUTION"
        print(f"\n{'='*60}")
        print(f"✅ Enable Memberships ({mode})")
        print(f"{'='*60}\n")
        ...
        # Filter out already active AND excluded users
        to_process = []
        for m in target_memberships:
            ...
            if m.get("status") == "active":
                print(f"   ℹ️  Membership {usr} @ {acc} is already active")
                continue
            
            to_process.append(m)

        if not to_process:
            print("\n✅ All matching memberships are either already active or skipped.")
            return

        print(f"\nFound {len(to_process)} membership(s) to enable:")
        for m in to_process:
             print(f" - User: {m.get('USER_ID')} | Account: {m.get('ACCOUNT_ID')}")

        # Confirmation
        if not self.dry_run:
            print(f"\n⚠️  WARNING: About to enable {len(to_process)} memberships")
        ...
        print("\nProcessing...")
        for m in to_process:
            if self.enable_membership(m["PK"], m["SK"]):
        ...
        print(f"📊 Summary")
        print(f"{'='*60}")
        print(f"Memberships enabled: {self.stats['enabled']}")
...
def main():
    parser = argparse.ArgumentParser(description="Enable user memberships")
    parser.add_argument("--user-id", help="User ID to enable memberships for")
    parser.add_argument("--account-id", help="Account ID to enable memberships for")
    ...
    enabler = MembershipEnabler(dry_run=not args.execute)
    enabler.run(args.user_id, args.account_id, except_ids)
