...
async def list_tours(
    ...
    items = svc.list_tours(group=workspace_id, tour_type=tour_type)
    return items

@router.post("", dependencies=[Depends(PermissionChecker(required_permissions=['admin']))])
async def create_tour(put_tour: PutTour, svc: TourService = Depends(get_tour_service)):
    item = svc.create(put_tour)
    return item

@router.get("/{tour_id}", dependencies=[Depends(PermissionChecker(required_permissions=['admin', 'user']))])
async def get_tour(tour_id: str, svc: TourService = Depends(get_tour_service)):
    item = svc.get(tour_id)
    if not item:
...
async def update_tour(tour_id: str, put_tour: PutTour, svc: TourService = Depends(get_tour_service)):
    existing_item = svc.get(tour_id)
    if not existing_item:
        raise HTTPException(status_code=404, detail=f"Tour {tour_id} not found")
    svc.update(tour_id, put_tour)
    return {"updated_tour_id": tour_id}

@router.delete("/{tour_id}", dependencies=[Depends(PermissionChecker(required_permissions=['admin']))])
async def delete_tour(tour_id: str, svc: TourService = Depends(get_tour_service)):
    svc.delete(tour_id)
...
async def generate_tour_presigned_urls(
    tour_id: str, files: list[FileSpec] = Body(..., embed=False), 
    svc: TourService = Depends(get_tour_service)
    ):
    tour = svc.get(tour_id)
    if not tour:
        raise HTTPException(status_code=404, detail=f"Tour {tour_id} not found")

    try:
        result = svc.generate_put_presigned_urls(tour_id=tour_id, files=files)
...
async def add_tour_images(tour_id: str, file_names: list[str] = Body(..., embed=False), svc: TourService = Depends(get_tour_service)):
    added_images = svc.add_images(tour_id, file_names)
...
async def update_booker_property(
        tour_id: str, booker_id: str, patch_property: PatchProperty, svc: TourService = Depends(get_tour_service)
        ):
    existing_item = svc.get(tour_id)
    if not existing_item:
        raise HTTPException(status_code=404, detail=f"Tour {tour_id} not found")

    updated_bookers = svc.update_booker_property(tour_id, booker_id, patch_property)
