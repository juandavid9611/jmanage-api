...
@router.post("")
async def create_user(create_user: CreateUser, svc: UserService = Depends(get_user_service)):
...
async def update_user(user_id: str, put_user: PutUser, svc: UserService = Depends(get_user_service)):
    existing_item = svc.get(user_id)
    if not existing_item:
        raise HTTPException(status_code=404, detail=f"User {user_id} not found")
    svc.update(user_id, put_user)
...
async def enable_user(user_id: str, svc: UserService = Depends(get_user_service)):
    svc.enable(user_id)
...
async def disable_user(user_id: str, svc: UserService = Depends(get_user_service)):
    svc.disable(user_id)
...
async def update_user_avatar_url(user_id: str, user_avatar: PutUserAvatar, svc: UserService = Depends(get_user_service)):
    svc.update_user_avatar_url(user_id, user_avatar)
...
async def get_user_metrics(user_id: str, svc: UserService = Depends(get_user_service)):
    item = svc.get_user_metrics(user_id)
    if not item:
        raise HTTPException(status_code=404, detail=f"User {user_id} not found")
    return item

@router.put("/{user_id}/metrics", dependencies=[Depends(PermissionChecker(required_permissions=['admin']))])
async def update_user_metrics(user_id: str, put_user_metrics: PutUserMetrics, svc: UserService = Depends(get_user_service)):
    existing_item = svc.get(user_id)
    if not existing_item:
        raise HTTPException(status_code=404, detail=f"User {user_id} not found")
    dict_metrics = svc.update_metrics(user_id, put_user_metrics)
...
async def get_late_arrives(user_id: str, svc: UserService = Depends(get_user_service)):
    items = svc.get_late_arrives(user_id)
    return items
